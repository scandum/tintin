...
struct iac_type iac_client_table [] =
{
	...
	{   3,  (unsigned char []) {IAC, WILL, TELOPT_MCCP4},                     &client_recv_will_mccp4         },
	...
	{  10,  (unsigned char []) {IAC, SB,   TELOPT_MCCP4, MCCP4_BEGIN_ENCODING, 'z', 's', 't', 'd', IAC, SE}, &client_init_mccp4 },
...
int client_translate_telopts(struct session *ses, unsigned char *src, int cplen)
{
	...
	if (ses->mccp2)
	{
        ...
        {
		ZSTD_inBuffer input = { src, (size_t)cplen, 0 };
		ZSTD_outBuffer output = { gtd->mccp_buf, (size_t)gtd->mccp_len, 0 };
		size_t retval;

		decompress:

		retval = ZSTD_decompressStream(ses->mccp4, &output, &input);

		if (ZSTD_isError(retval))
		{
			tintin_puts2(ses, "");
			tintin_printf2(ses, "#COMPRESSION ERROR, ZSTD: %s, DISABLING MCCP4.", ZSTD_getErrorName(retval));
		...
		else if (output.pos == output.size)
		{
			...
			output.dst = gtd->mccp_buf;
			output.size = gtd->mccp_len;

			goto decompress;
		}
		else if (retval == 0) // Z_STREAM_END equivalent
		{
			client_telopt_debug(ses, "#COMPRESSION END, DISABLING MCCP4.");

			cnt = output.pos;

			cpsrc = src + input.pos;
			cplen = cplen - input.pos;
		...
		else // Z_OK equivalent
		{
			ses->mccp4_in += cplen;
			ses->mccp4_out += output.pos;
