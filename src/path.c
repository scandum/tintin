...
DO_PATH(path_get)
{
	...
	{
		set_nest_node_ses(ses, arg2, "%d", root->used);

		show_message(ses, LIST_COMMAND, "#PATH GET: PATH LENGTH {%d} SAVED TO {%s}.", root->used, arg2);
	}
	else if (is_abbrev(arg1, "MAPPING"))
	{
		node = set_nest_node_ses(ses, arg2, "%d", HAS_BIT(ses->flags, SES_FLAG_PATHMAPPING) ? 1 : 0);

		show_message(ses, LIST_COMMAND, "#PATH GET: PATH MAPPING {%d} SAVED TO {%s}.", node->arg2, arg2);
	}
	else if (is_abbrev(arg1, "POSITION"))
	{
		set_nest_node_ses(ses, arg2, "%d", root->update + 1);

		show_message(ses, LIST_COMMAND, "#PATH GET: PATH POSITION {%d} SAVED TO {%s}.", root->update + 1, arg2);
	}
	else if (is_abbrev(arg1, "RUNNING"))
	{
		node = set_nest_node_ses(ses, arg2, "%.2f",
			root->update >= root->used ? 0 :
			root->list[root->update]->val64 == 0 ? 0 :
			(root->list[root->update]->val64 - gtd->utime) < 10000 ? 0.01 :
			(root->list[root->update]->val64 - gtd->utime) / 1000000.0);

		show_message(ses, LIST_COMMAND, "#PATH GET: PATH RUNNING {%s} SAVED TO {%s}.", node->arg2, arg2);
	}
	else
	{
		show_error(ses, LIST_COMMAND, "#SYNTAX: #PATH GET {INFO|LENGTH|MAPPING|POSITION|RUNNING} <VARIABLE>");
